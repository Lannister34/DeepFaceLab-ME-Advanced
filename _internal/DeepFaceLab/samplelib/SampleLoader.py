...
class SampleLoader:
    ...
    def get_person_id_max_count(samples_path):
        ...
        try:
            ...
            io.log_err(f"An error occurred while loading the sample library {str(samples_path)}, {traceback.format_exc()}")

        if samples is None:
            raise ValueError("Packed face set not found")
        persons_name_idxs = {}
        ...
        if sample_type == SampleType.IMAGE:
            ...
            if  samples[sample_type] is None:
                try:
                    ...
                    io.log_err(f"Error loading samplelib.PackedFaceset.load {str(samples_path)}, {traceback.format_exc()}")
...
class FaceSamplesLoaderSubprocessor(Subprocessor):
    ...
    def __init__(self, image_paths ):
        ...
        super().__init__('Face sample loading', FaceSamplesLoaderSubprocessor.Cli, 60)

    #override
    def on_clients_initialized(self):
        io.progress_bar ("Instance loading", len (self.image_paths))
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if dflimg is None or not dflimg.has_data():
                self.log_err (f"Face sample loading: {filename} is not a dfl image file.")
                data = None
            else:
