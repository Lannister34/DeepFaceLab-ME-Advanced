...
class SegIEPolys():
    ...
    @staticmethod
    def load_from_xseg(xseg_mask, image_height):
        ie_polys = SegIEPolys()
        if xseg_mask is not None:
            xseg_res = 256
            polys = []
            contours, _ = cv2.findContours(xseg_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            scale_factor = image_height / xseg_res
            for contour in contours:
                pts = []
                for point in contour:
                    x, y = point[0]
                    pts.append([x*scale_factor, y*scale_factor])
                pts_array = np.array(pts)
                if len(pts_array) > 2:  # Ensure the contour has enough points to form a polygon
                    poly = SegIEPoly(SegIEPolyType.INCLUDE, pts_array)
                    ie_polys.polys.append(poly) # Create a polygon object (You may need to define Poly class)
        ie_polys.sort()
