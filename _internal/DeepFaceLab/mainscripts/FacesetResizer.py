...
def process_folder (dirpath):
    ...
    if PackedFaceset.path_contains(dirpath):
        io.log_info (f'\n{dirpath} Contains packaged face sets! Please unpack first.\n')
        return
    
    image_size = io.input_int(f"New image size", 512, valid_range=[128,2048])
    
    face_type = io.input_str ("Changing the face type", 'same', ['h','mf','f','wf','head','same']).lower()
    if face_type == 'same':
        face_type = None
    else:
    ...
    io.log_info (f"Resize faces in {dirpath_parts}")
    ...
    if is_merge:
        io.log_info (f"Copy the processed files to {dirpath_parts}")
