...
class ExtractSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.log_info (f"operating {client_dict['device_name'] }")
...
def main(detector=None,
    ...
    if not extract_from_video:
        if not manual_output_debug_fix and len(output_images_paths) > 0:
            if len(output_images_paths) > 128:
                continue_extraction = io.input_bool ("Continued extraction?", True, help_message="The extraction can continue, but the same options must be specified again.")
            ...
            if len(output_images_paths) > 128 and continue_extraction:
                try:
                    ...
                    io.log_err("Error getting last index. Unable to continue extraction .")
                    return
    ...
    if face_type is None:
        face_type = io.input_str ("Face type", 'wf', ['f','wf','head'], help_message="Full face / whole face / head. Full Face / Whole Face / Head. A full face includes the entire face area, including the forehead. Head includes the entire head, but requires the use of XSeg for both source and target face sets.").lower()
        face_type = {'f'  : FaceType.FULL,
                     'wf' : FaceType.WHOLE_FACE,
                     'head' : FaceType.HEAD}[face_type]

    if max_faces_from_image is None:
        max_faces_from_image = io.input_int(f"Max number of faces from image", 0, help_message="If the source face set you are extracting contains frames with a large number of faces, it is recommended to set the Maximum Number of Faces to 3 to speed up the extraction. 0 - Unlimited.")

    if image_size is None:
        image_size = io.input_int(f"Image size", 512 if face_type < FaceType.HEAD else 768, valid_range=[256,2048], help_message="Output image size. The larger the image size, the less effective the face enhancement will be. Use values higher than 512 only if the source image is sufficiently clear and there is no need for face enhancement.")

    if jpeg_quality is None:
        jpeg_quality = io.input_int(f"Jpeg quality", 90, valid_range=[1,100], help_message="JPEG Quality. The higher the quality, the larger the output file size.")

    if extract_from_video and fps is None:
        fps = io.input_int ("Enter FPS", 0, help_message="Extracts frames per second from the video. 0 - Using the full frame rate")

    if detector is None:
        io.log_info ("Select Detector Type.")
        io.log_info ("[0] S3FD")
    ...
    if output_debug is None:
        output_debug = io.input_bool (f"Write debug image to {output_debug_path.name}?", False)
    ...
    if not extract_from_video:
        if manual_output_debug_fix:
            if not output_debug_path.exists():
                io.log_err(f'{output_debug_path} Not found, please reextract faces with debug image option')
                return
            else:
                detector = 'manual'
                io.log_info('Frames deleted from the _debug directory are being reextracted.')
    ...
    if not extract_from_video:
        ...
        if images_found != 0:
            if detector == 'manual':
                if not extract_from_video:
                    io.log_info ('Perform manual extraction...')
                    ...
                    io.log_info ('Implementation of the third treatment...')
            ...
            if manual_fix:
                if all ( np.array ( [ d.faces_detected > 0 for d in data] ) == True ):
                    io.log_info ('All faces are detected, no need to fix them manually.')
                else:
                    fix_data = [ ExtractSubprocessor.Data(filepath=d.filepath) for d in data if d.faces_detected == 0 ]
                    io.log_info ('Perform manual restoration on %d images...' % (len(fix_data)) )
    ...
    io.log_info ('-------------------------')
    io.log_info (f"Number of images found:        {images_found if not extract_from_video else 'No frame detected. (You are using Extract from Video mode)'}")
    io.log_info (f'Number of detected faces:      {faces_detected}')
    io.log_info ('-------------------------')
