...
class FacesetEnhancerSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            intro_str = 'operating %s.' % (client_dict['device_name'])
        ...
        def process_data(self, filepath):
            try:
                ...
                self.log_err (f"An exception occurred while processing file {filepath}. Error: {traceback.format_exc()}")
...
def process_folder ( dirpath, cpu_only=False, force_gpu_idxs=None ):
    ...
    if PackedFaceset.path_contains(dirpath):
        io.log_info (f'\n{dirpath} Contains the packaged faceset! Please unpack first.\n')
        return
    ...
    io.log_info (f"Enhanced face set {dirpath_parts}")
    ...
    is_merge = io.input_bool (f"\r\nMerge {output_dirpath_parts} into {dirpath_parts} ?", True)
    if is_merge:
        io.log_info (f"Copy the processed file to the {dirpath_parts}")
        ...
        io.log_info (f"Delete {output_dirpath_parts}")
