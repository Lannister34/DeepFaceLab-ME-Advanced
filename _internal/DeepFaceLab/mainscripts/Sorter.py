    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if dflimg is None or not dflimg.has_data():
                self.log_err (f"{filepath.name} Not a DFL image file")
...
def sort_by_blur(input_path, arg):
    io.log_info (f"Sort by {arg[Arguments.DESC.value]}...")
...
class HistSsimSubprocessor(Subprocessor):
    ...
    def process_info_generator(self):
        ...
        io.log_info(f'Using {cpu_count} threads.')
        for i in range(cpu_count):
            yield 'CPU%d' % (i), {'i':i}, {}
    ...
    #override
    def on_data_return (self, host_dict, data):
        raise Exception("Data processing failed. Please reduce the number of images and try again.")
...
def sort_best(input_path, faster=False):
    target_count = io.input_int ("Number of target faces?", 2000)

    if faster:
        io.log_info("Use a faster algorithm. Faces will be sorted by source rectangular region instead of fuzziness.")
    ...
    for g in io.progress_bar_generator ( range(grads), "Side-by-side sorting"):
        yaw = grads_space[g]
    ...
    if len(img_list) != 0:
        for i in io.progress_bar_generator( [*range(len(img_list))], "Renaming", leave=False):
            ...
            try:
                ...
                io.log_info ('Rename Failure %s' % (src.name) )
...
def process_by_face_yaw(filepath):
    ...
    if dflimg is None or not dflimg.has_data():
        print(f"{path.name} is not a DFL image file. It is being deleted...")
...
def sort_starter(input_path, arg):
    ...
    cpus = io.input_int('Number of CPU cores to be used', 
                    help_message='If the default option is selected, it will use all CPU cores and will slow down the computer',
...
def main (input_path, sort_by_method=None):
    io.log_info ("Run the sorting tool.\r\n")

    if PackedFaceset.path_contains(input_path):
        io.log_info (f'\n{input_path} Contains the packaged faceset! Please unpack it first.\n')
        return

    if sort_by_method is None:
        io.log_info(f"Selection sorting method:")
